...
def apply_error_model(
    ...
    sun_mask = hr > 0
    delta_hr = delta * hr
    error[sun_mask] += delta_hr[sun_mask] + epsilon[sun_mask]
...
def run_forecasts1(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['alpha'] = 1.2 - 0.4 * np.cos(2 * np.pi * (df['day_of_year'] - 172) / 365) + np.random.normal(mu, sigma, len(df))
...
def run_forecasts2(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Seasonal alpha: varies with time of year
    df['alpha'] = 0.8 + 0.4 * np.cos(2 * np.pi * (df['day_of_year'] - 172) / 365) + np.random.normal(mu, sigma, len(df))


    # Constant delta
    df['delta'] = -0.04 * np.sin(2 * np.pi * df['day_of_year'] / 365) + np.random.normal(mu2, sigma2, len(df))
...
def run_forecast3(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['alpha'] = 1 + 0.5 * cloud_index + np.random.normal(mu, sigma, len(df))
    
    # Constant delta
    df['delta'] = 0.02 + 0.05 * cloud_index + np.random.normal(mu2, sigma2, len(df))
...
def run_forecast4(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['alpha'] = 1.0 + 0.00010 * rolling_std + np.random.normal(mu, sigma, len(df)) # scaling factor to exaggerate effect slightly
...
def run_forecast5(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Constant alpha
    df['alpha'] = 1.0 + np.random.normal(mu, sigma, len(df))
...
def run_forecast6(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['alpha'] = 1.0  + np.random.normal(mu, sigma, len(df)) # could exceed 1.0 or drop below 0.5 depending on error
    
    # Delta: oppose the direction of previous forecast residual
    df['delta'] = -0.05  + np.random.normal(mu2, sigma2, len(df))
